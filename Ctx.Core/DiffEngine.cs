...
public sealed class DiffEngine : IDiffEngine
{
    public ContextDiff Diff(ContextCommit? previous, RepositorySnapshot current)
    {
        ...
        var epics = DiffEntities(previousWorkingContext?.Epics ?? Array.Empty<Epic>(), currentWorkingContext.Epics ?? Array.Empty<Epic>(), item => item.Id.Value, item => $"{item.Title} [{item.State}]");
        ...
        return new ContextDiff(previous?.Id, currentWorkingContext.HeadCommitId, decisions, hypotheses, evidence, tasks, conclusions, runbooks, triggers, Array.Empty<CognitiveConflict>(), summary, epics);
    }
