...
- 公共仓库现在携带经过清洗的 agriculture bootstrap example packs（`v1`、`v2`、`v3`、`v4`），包括 plans 和 real-testing notes，同时将本地 `.ctx` workspaces 保持在已发布 surface 之外。
- 公共 helper 与技术文档现已与发布边界对齐，使 release operators 与 agents 始终锚定在预期的 public-safe workflow 上。
